...
function registerCronJobs() {
        ...
        try {
            ...
            for (const lead of res.rows) {
                try {
                    ...
                    const statusAtendimento = lead.status_atendimento;
                    // Busca e incrementa o contador de tentativas no Redis
                    const redisKey = `followup_count:${phone}`;
                    const countRaw = await redis_1.default.get(redisKey);
                    const attempt = (parseInt(countRaw || '0', 10) || 0) + 1;
                    await redis_1.default.set(redisKey, String(attempt), 'EX', 60 * 60 * 24 * 14); // TTL 14 dias
                    if (attempt > 3) {
                        // Etapa 4+: encerrar follow-up, marcar como NUTRICAO
                        await client.query(`UPDATE leads SET situacao = 'nutricao', atualizado_em = NOW() WHERE id = $1`, [lead.id]);
                        await client.query(`
    ...
    node_cron_1.default.schedule('0 * * * *', async () => {
        const log = logger_1.cronLogger.withTrace(`cron-integra-${Date.now().toString(36)}`);
        const now = new Date();
        const diaAtual = now.getDate();
        const horaAtual = now.getHours();
        try {
            const robos = await db_1.default.query(`
    ...
    // ============================
    node_cron_1.default.schedule('0 12,15,18,21 * * *', async () => {
        const log = logger_1.cronLogger.withTrace(`cron-pos-reuniao-${Date.now().toString(36)}`);
        ...
        try {
            ...
            const msg = `📋 *Confirmação de Reuniões — ${horaAtual}*\n\n` +
                `${linhas.join('\n')}\n\n` +
    ...
    // ============================
    node_cron_1.default.schedule('0 * * * *', async () => {
        const log = logger_1.cronLogger.withTrace(`cron-redflag24-${Date.now().toString(36)}`);
        const client = await db_1.default.connect();
        try {
            const res = await client.query(`
                SELECT
            ...
            for (const lead of res.rows) {
                ...
                if (await redis_1.default.get(redisKey))
                    continue; // já notificado
                await client.query(`UPDATE leads SET situacao = 'red_flag', atualizado_em = NOW() WHERE id = $1`, [lead.id]);
                await client.query(`
                ...
                `, [lead.id]);
                await redis_1.default.set(redisKey, '1', 'EX', 60 * 60 * 24 * 7); // TTL 7 dias
                if (attendantPhone) {
            ...
            if (flagged > 0)
                log.info(`${flagged} lead(s) marcados como red_flag por 24h sem procuração`);
        }
        catch (err) {
    ...
    // ============================
    node_cron_1.default.schedule('0 9,18 * * *', async () => {
        const log = logger_1.cronLogger.withTrace(`cron-cert-${Date.now().toString(36)}`);
        try {
            const notAfter = (0, serpro_1.getCertNotAfter)();
            if (!notAfter) {
            ...
            if (await redis_1.default.get(dedupKey))
                return;
            ...
            // senão, semanal.
            await redis_1.default.set(dedupKey, String(daysLeft), 'EX', critical ? 60 * 60 * 8 : 60 * 60 * 24 * 7);
