...
async function sendFallback(sender, userPhone) {
    try {
        ...
        logger_1.debounceLogger.error('Falha crítica no fallback (envio mensagem):', err);
    }
    // Notifica o atendente humano independente do estado do BD
    try {
        const { callAttendant } = await Promise.resolve().then(() => __importStar(require('../ai/server-tools')));
        await callAttendant(userPhone, '[FALHA DE SISTEMA] Bot falhou ao processar a mensagem. Cliente notificado.');
    }
    catch (err) {
        logger_1.debounceLogger.error('Falha ao notificar atendente no fallback:', err);
    }
    ...
    try {
        ...
        logger_1.debounceLogger.error('Falha ao atualizar usuário no fallback (BD pode estar fora):', err);
    }
}
// ==================== API Pública ====================
/**
