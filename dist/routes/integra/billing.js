...
router.get('/integra/billing', async (req, res) => {
    const mes = req.query.mes || new Date().toISOString().slice(0, 7); // 'YYYY-MM'
    const [ano, mesNum] = mes.split('-').map(Number);
    if (!ano || !mesNum)
        return void res.status(400).json({ error: 'Formato mes inválido. Use YYYY-MM' });
    ...
    try {
        ...
        const detalhe = await (0, db_1.query)(`
            SELECT
                e.id          AS empresa_id,
                e.cnpj,
                e.razao_social,
        ...
        const totais = await (0, db_1.query)(`
            SELECT
        ...
        `, [inicioMes, fimMes]);
        // Preços vigentes
        const precos = await (0, db_1.query)(`SELECT * FROM integra_precos ORDER BY tipo_robo`);
        res.json({
            mes,
...
router.patch('/integra/billing/precos/:tipo_robo', async (req, res) => {
    ...
    const result = await (0, db_1.query)(`UPDATE integra_precos
         SET preco_unitario = $1, updated_at = NOW()
         WHERE tipo_robo = $2
         RETURNING *`, [preco_unitario, req.params.tipo_robo]);
    if (!result.rows.length)
        return void res.status(404).json({ error: 'Tipo de robô não encontrado' });
