...
function fullLeadSelect(keyParam) {
    return `
    SELECT
...
router.get('/leads/user/:phone', async (req, res) => {
    ...
    try {
        const result = await (0, db_1.query)(`${fullLeadSelect('$2')} WHERE ${col} = $1`, [identifier, key]);
...
router.put('/leads/user/:phone', async (req, res) => {
    ...
    const body = req.body;
    const { nome_completo, telefone, email, senha_gov, cnpj, tipo_negocio, possui_socio, tipo_divida, valor_divida_municipal, valor_divida_estadual, valor_divida_federal, valor_divida_pgfn, valor_divida_ativa, faturamento_mensal, observacoes, interesse_ajuda, teria_interesse, calculo_parcelamento, data_reuniao, } = body;
    ...
    try {
        const leadId = await (0, db_1.withClient)(async (client) => {
            ...
            let id;
            if (idRes.rowCount === 0) {
                const ins = await client.query(`INSERT INTO leads (telefone, nome_completo, email, senha_gov_enc, data_cadastro, atualizado_em)
           ...
           RETURNING id`, [
             ...
             atualizado_em  = NOW()
           WHERE id = $1`, [id, nome_completo, telefone, email, senha_gov || null, key]);
            }
            await client.query(`UPDATE leads SET
           ...
           atualizado_em          = NOW()
         WHERE id = $1`, [id, cnpj, tipo_negocio, faturamento_mensal, finalInteresse,
            ...
            await client.query(`INSERT INTO leads_processo (lead_id, envio_disparo, data_controle_24h, observacoes)
         VALUES ($1, 'a1', NOW(), $2)
         ON CONFLICT (lead_id) DO UPDATE SET
           observacoes     = COALESCE(EXCLUDED.observacoes, leads_processo.observacoes),
           envio_disparo   = 'a1',
           data_controle_24h = NOW(),
           updated_at      = NOW()`, [id, observacoes]);
            if (data_reuniao) {
                await client.query(`INSERT INTO leads_processo (lead_id, data_reuniao, status_atendimento)
           VALUES ($1, $2, 'reuniao')
           ON CONFLICT (lead_id) DO UPDATE SET
             data_reuniao       = EXCLUDED.data_reuniao,
             status_atendimento = 'reuniao',
             updated_at         = NOW()`, [id, data_reuniao]);
            }
        ...
        const updated = await (0, db_1.query)(`${fullLeadSelect('$2')} WHERE l.id = $1`, [leadId, key]);
...
router.post('/leads/bulk-update', async (req, res) => {
    ...
    const updateColumnRaw = body?.update?.column ?? body.updateColumn ?? column;
    ...
    switch (operator) {
        case 'in':
            whereSql = `${whereColRef}::text = ANY($${params.length + 1})`;
            whereParams.push(values);
            break;
        case 'not_in':
            whereSql = `NOT (${whereColRef}::text = ANY($${params.length + 1})) OR ${whereColRef} IS NULL`;
            whereParams.push(values);
            break;
        case 'is_empty':
            whereSql = `${whereColRef} IS NULL OR ${whereColRef}::text = ''`;
            break;
        case 'is_not_empty':
            whereSql = `${whereColRef} IS NOT NULL AND ${whereColRef}::text <> ''`;
            break;
...
router.post('/leads/bulk-delete', async (req, res) => {
    ...
    if (body.qualification !== undefined) {
        ...
        try {
            const result = await (0, db_1.query)(sql, qual === 'vazio' ? [] : [qual]);
    ...
    switch (operator) {
        case 'in':
            whereClause = `${realColumn}::text = ANY($1)`;
            params = [values];
            break;
        case 'not_in':
            whereClause = `NOT (${realColumn}::text = ANY($1)) OR ${realColumn} IS NULL`;
            params = [values];
            break;
        case 'is_empty':
            whereClause = `${realColumn} IS NULL OR ${realColumn}::text = ''`;
            break;
        case 'is_not_empty':
            whereClause = `${realColumn} IS NOT NULL AND ${realColumn}::text <> ''`;
            break;
        default: return void res.status(400).json({ error: 'Operador inválido' });
    }
...
router.put('/leads/update-fields', async (req, res) => {
    ...
    for (const [rawKey, v] of Object.entries(updates)) {
        ...
        if (leadsColumns.has(k))
            leadsFields[k] = value;
        else if (processoColumns.has(k))
            processoFields[k] = value;
    }
    ...
    try {
        await (0, db_1.withClient)(async (client) => {
            ...
            if (Object.keys(processoFields).length > 0) {
                ...
                await client.query(`INSERT INTO leads_processo (${cols}) VALUES (${placeholders})
           ON CONFLICT (lead_id) DO UPDATE SET ${updateSet}, updated_at=NOW()`, [leadId, ...Object.values(processoFields)]);
            }
            await client.query('COMMIT');
        });
