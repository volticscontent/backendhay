...
async function runVendedorAgent(message, context) {
    ...
    const tools = [
        ...
        { name: 'update_user', description: 'Atualizar dados do usuário.', parameters: { type: 'object', properties: { situacao: { type: 'string', enum: ['nao_respondido', 'desqualificado', 'qualificado', 'cliente', 'atendimento_humano'] }, observacoes: { type: 'string' }, tipo_negocio: { type: 'string' }, tem_divida: { type: 'boolean' }, valor_divida_federal: { type: 'string' }, cnpj: { type: 'string' }, razao_social: { type: 'string' }, faturamento_mensal: { type: 'string' } }, additionalProperties: true }, function: async (args) => await (0, server_tools_1.updateUser)({ telefone: context.userPhone, ...args }) },
