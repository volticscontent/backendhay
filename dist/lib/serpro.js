...
async function request(urlStr, options, body) {
...
async function getSerproTokens() {
    ...
    if (response.access_token && response.jwt_token) {
        cachedTokens = {
...
async function consultarServico(nomeServico, cnpj, options = {}) {
    ...
    const dadosServico = { cnpj: cnpjNumero };
    if (['PGMEI', 'SIMEI', 'DIVIDA_ATIVA', 'PGDASD', 'DCTFWEB', 'CAIXA_POSTAL'].includes(nomeServico)) {
        ...
        if (nomeServico === 'DCTFWEB') {
            ...
            if (!dadosServico.anoPA)
                dadosServico.anoPA = new Date().getFullYear().toString();
    ...
    logger_1.serproLogger.info(`Consultando ${nomeServico} para CNPJ ${cnpjNumero}`);
