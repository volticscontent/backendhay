...
exports.isScheduleHallucination = isScheduleHallucination;
exports.sanitizeHallucination = sanitizeHallucination;
...
const SAFE_PGFN_FALLBACK = 'No momento não consegui confirmar a situação na Dívida Ativa da União (PGFN) por uma instabilidade momentânea nos sistemas da Receita. Vou reconsultar e, se necessário, encaminhar a um especialista.';
/** True se o trecho contém a alucinação de horário da PGFN/Receita. */
function isScheduleHallucination(text) {
 ...
 */
function sanitizeHallucination(text) {
    if (!text || !isScheduleHallucination(text))
        return text;
    ...
    if (!removedAny || kept.length === 0)
        return SAFE_PGFN_FALLBACK;
