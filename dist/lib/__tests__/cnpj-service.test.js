...
describe('CNPJService', () => {
    ...
    describe('consultarCNPJ', () => {
        ...
        it('deve usar cache em consultas repetidas', async () => {
            ...
            mockedAxios.get.mockResolvedValueOnce({ data: brasilAPIData });
            // Primeira consulta
            const result1 = await cnpj_service_1.cnpjService.consultarCNPJ(validCNPJ);
            expect(result1.cached).toBe(false);
            // Segunda consulta (deve usar cache)
            const result2 = await cnpj_service_1.cnpjService.consultarCNPJ(validCNPJ);
        ...
        it('deve retornar erro para CNPJ inválido', async () => {
            const result = await cnpj_service_1.cnpjService.consultarCNPJ(invalidCNPJ);
        ...
        it('deve retornar erro quando CNPJ não for encontrado', async () => {
            ...
            const result = await cnpj_service_1.cnpjService.consultarCNPJ(validNotFoundCNPJ);
            expect(result.success).toBe(false);
        ...
        it('deve aplicar rate limiting', async () => {
            ...
            for (let i = 0; i < 10; i++) {
                const result = await cnpj_service_1.cnpjService.consultarCNPJ(validCNPJ, clientIp);
                expect(result.success).toBe(true);
            }
    ...
    describe('consultarLote', () => {
        it('deve consultar múltiplos CNPJs em lote', async () => {
            ...
            const results = await cnpj_service_1.cnpjService.consultarLote(cnpjs);
    ...
    describe('cache', () => {
        it('deve armazenar e recuperar dados do cache', async () => {
            ...
            await cnpj_service_1.cnpjService.consultarCNPJ(validCNPJ);
            // Verifica estatísticas do cache
            const stats = cnpj_service_1.cnpjService.getCacheStats();
            expect(stats.size).toBe(1);
            // Limpa o cache
            cnpj_service_1.cnpjService.clearCache();
            const statsAfterClear = cnpj_service_1.cnpjService.getCacheStats();
            expect(statsAfterClear.size).toBe(0);
        });
    });
});
//# sourceMappingURL=cnpj-service.test.js.map