...
// ─── Chamada ao backend ────────────────────────────────────────────────────────
async function call(service, extra = {}) {
    const t0 = Date.now();
...
function resumo(body) {
    ...
    const b = body;
    // CCMEI/SIMEI
    if (b.nomeEmpresarial)
        return `nomeEmpresarial: ${b.nomeEmpresarial}`;
    // PGMEI/DIVIDA_ATIVA
    if (b.situacaoContribuinte)
        return `situacaoContribuinte: ${b.situacaoContribuinte}`;
    if (b.primary && typeof b.primary === 'object')
        return resumo(b.primary);
    // Mensagens de erro do Serpro
    if (Array.isArray(b.mensagens) && b.mensagens.length)
        return b.mensagens.map((m) => `[${m.codigo}] ${m.texto}`).join(' | ').slice(0, 160);
    // dados como JSON string
    if (b.dados && typeof b.dados === 'string') {
        try {
            return resumo(JSON.parse(b.dados));
        }
        catch {
            return b.dados.slice(0, 120);
        }
    }
...
// ─── Testes individuais ────────────────────────────────────────────────────────
async function testar(service, extra = {}, skipSe) {
    if (skipSe) {
...
async function run() {
    ...
    if (CPF) {
        ...
        resultados.push(await testar('SIT_FISCAL_RELATORIO', { cpf: CPF, protocoloRelatorio: sitfisProtocolo }, sitfisProtocolo ? undefined : 'Protocolo SITFIS não obtido no passo anterior'));
        resultados.push(await testar('CND', { cpf: CPF, protocoloRelatorio: sitfisProtocolo }, sitfisProtocolo ? undefined : 'Protocolo SITFIS não obtido — CND depende do SITFIS'));
    }
    else {
    ...
    resultados.push(await testar('PGDASD', {}, 'Requer numeroDas de uma consulta prévia — rode manualmente no painel após PGMEI'));
...
//# sourceMappingURL=test-serpro-apis.js.map